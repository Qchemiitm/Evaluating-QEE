"""
The purpose of this module is to perform minimum configuration search for H2 molecule by estimating the bond length for minimum ground state energy.
...
def setup_h2_molecule(distance, debug=False) -> EnergyEstimator:
    ## first setup geometry
    geometry = geometry = [["H", [0, 0, 0]], ["H", [0, 0, distance]]]
    log.info(f"The geometry of molecule H2 is: {geometry}")

    ## return energy estimator object
    ee = EnergyEstimator(geometry, debug=debug)
    log.info(f"H2 molecule setup for distance {distance}.")
    return (ee)

# identify qubit hamiltonian 
def h2_qubit_hamiltonian(ee, mapper="QEE", debug=False):
    # set QEE mapper
...
"""
Following function is just a test function and is not really used elsewhere in the program
"""
def build_custom_ansatz(num_qubits):
    ## here we build a specific ansatz for H2 molecule
    qbits = QuantumRegister(num_qubits)
    qc = QuantumCircuit(qbits,name="h2_custom")
    myParam = Parameter('theta')
    qc.ry(theta=np.pi/2, qubit=qbits[0])
    qc.rx(theta=-np.pi/2, qubit=qbits[1])
    qc.cx(control_qubit=qbits[0],target_qubit=qbits[1])
    qc.rz(phi=myParam, qubit=qbits[1])
    qc.cx(control_qubit=qbits[0],target_qubit=qbits[1])
    qc.ry(theta=-np.pi/2, qubit=qbits[0])
    qc.rx(theta=np.pi/2, qubit=qbits[1])
...
def main():
    ...
    ***************************************************************
    ...
    for dist in distance_list:
        ...
        ee = setup_h2_molecule(distance=dist, debug=is_debug)
        qubit_op = h2_qubit_hamiltonian(ee, mapper=mapper, debug=is_debug)
        ...
        if outputpath_exists == True and is_ansatz_printed == False:
            qcex.draw_circuit(in_filename=outputfilepath + '/' + 'ansatz_gradient_2_' + mapper + '_' + molecule + '.png', is_decompose=True, in_output_type='mpl')
            #ansatz.decompose().draw(output='mpl',filename=outputfilepath + '/' + 'ansatz_' + mapper + '_' + molecule + '.png')
            log.info(f"Ansatz printed. Check for file ansatz_gradient_2_{mapper}_{molecule}.png in the output directory.")
    ...
    """
    ***************************************************************
    Phase II: In this phase, we use the gradient descent / steepest descent method to identify minimum energy configuration.
    ***************************************************************
    """
