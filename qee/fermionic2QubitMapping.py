#from qiskit_nature.operators.second_quantization import fermionic_op
...
def operator2WeightedPauliOperator(operator, coef=1, num_qubits=None):
    ...
    #return ([PauliOp(label2Pauli(label),coeff=coef) for (coef, label) in pauli_labels])
    return ([PauliOp(Pauli(label),coeff=coef) for (coef, label) in pauli_labels])
...
def complete_mapping(naive_mapping):
    ...
    for k, w in naive_mapping.items():
        ...
        if (k1 != k2) and ((k2, k1) not in naive_mapping.keys()):
            new_k = (k2, k1)
            #paulis = list(map(lambda x: [np.conj(x[0]), x[1]], w))
            #mapping[new_k] = WeightedPauliOperator(paulis)
            paulis = []
            for op in w:
                # adjoint function can be used instead of np.conj used in the earlier codebase
                paulis.append(op.adjoint()) 
            mapping[new_k] = paulis
    return mapping

def pauliOp_to_list(p):
    """
    This helper function added by Amit S. Kesari to convert PauliOp(Pauli(), complex Coefficient) 
    to a list i.e. [complex coefficient, Pauli()]
    """
    return([p.coeff, p.primitive])

def convert_mapping(mapping):
    """
    This function added by Amit S. Kesari to return mapping dictionary values as 
    list(list(complex coeff, Pauli())) instead of list(PauliOp(primtive Pauli, complex coeff))
    """
    mapping_final = {}
    for key in mapping:
        pauliop_list = []
        for item in mapping[key]:
            pauliop_list.append(pauliOp_to_list(item))
        mapping_final[key] = pauliop_list
    return(mapping_final)

def fermionic2QubitMapping(num_so, num_e, mode = 'rhf', labeling_method = default_labeling):
    """
    ...
    log.info(f"Number of qubits identified for QEE mapper: {num_qubits}")
    log_count_entry_operators = [(k, len(val)) for k, val in entry_operators.items()]
    log.debug(f"Number of entry Operators: {log_count_entry_operators}")
    
    naive_mapping = get_naive_mapping_from_entry(entry_operators, num_qubits)
    mapping = complete_mapping(naive_mapping)
    log_count_mapping = [(k, len(val)) for k, val in mapping.items()]
    log.debug(f"Number of mapping elements: {log_count_mapping}")
    # Following method call added by Amit S. Kesari to convert [PauliOp()] to [[]]
    #mapping_final = convert_mapping(mapping)
